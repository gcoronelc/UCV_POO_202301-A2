...
public class EducaService {
	...
	public int procMatricula(int idAlumno, int idCurso, double precio, int cuotas) {
		...
		try {
			...
			// 2.- Validar alumno
			validarAlumno(idAlumno); // Verificar si existe código del alumno
	...
	private void validarMatriculaNoExista(int idAlumno, int idCurso) throws SQLException {
		String SQL = "SELECT COUNT(1) FILAS FROM MATRICULA WHERE alu_id=? AND cur_id=?";
		PreparedStatement pstm = cn.prepareStatement(SQL);
		pstm.setInt(1, idAlumno);
